// NSE Capital Market Multicast UDP Receiver - Message 6571 Only
// 
// FOCUS: Only process message code 6571 (BC_NORMAL_MKT_PREOPEN_ENDED)
// OUTPUT: csv_output/message_6571_TIMESTAMP.csv
//
// USAGE:
// ======
// Run: go run message_6571_live.go
// Output: csv_output/message_6571_TIMESTAMP.csv
//
// Protocol Reference: NSE CM NNF Protocol v6.3
// Structure: BCAST_VCT_MESSAGES (298 bytes)
// Session: Preopen → Regular transition - Preopen period ended notification
...
// =============================================================================
// MESSAGE STRUCTURE FOR 6571
// =============================================================================

// Message6571 - BC_NORMAL_MKT_PREOPEN_ENDED (Preopen Ended Messages)
// Per NSE CM Protocol - BCAST_VCT_MESSAGES structure
// Total packet: 298 bytes
type Message6571 struct {
	TransactionCode uint16   // Always 6571
	...
	Msg             [240]byte // 240 bytes - Preopen ended message content
}
...
var (
	...
	// 6571 specific counters
	message6571Count   int64
	message6571Saved   int64
	
	// CSV file for 6571
	csvFile6571        *os.File
	csvWriter6571      *csv.Writer
...
func main() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("NSE CM UDP Receiver - Message 6571 (BC_NORMAL_MKT_PREOPEN_ENDED)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
	fmt.Printf("Listening for message code 6571 (0x19AB in hex)\n")
	fmt.Printf("Purpose: Preopen period ended notification\n")
	fmt.Printf("Session: Preopen → Regular transition (9:15 AM)\n")
	...
	// Initialize CSV file for 6571
	if err := initialize6571CSV(); err != nil {
		log.Fatalf("Failed to initialize CSV: %v", err)
		return
	}
	defer func() {
		if csvWriter6571 != nil {
			csvWriter6571.Flush()
		}
		if csvFile6571 != nil {
			csvFile6571.Close()
...
func initialize6571CSV() error {
	timestamp := time.Now().Format("20060102_150405")
	filename := fmt.Sprintf("message_6571_%s.csv", timestamp)
	...
	csvFile6571 = file
	csvWriter6571 = csv.NewWriter(file)
...
func processUDPPacket(data []byte) {
	...
	// Offset 10-11: TransactionCode (SHORT 2 bytes) ← 6571 for BC_NORMAL_MKT_PREOPEN_ENDED
	...
	// Only process message 6571
	if transactionCode != 6571 {
		return
	}

	// Process the message (finalData already has 8 bytes skipped)
	process6571Message(finalData)
}

// =============================================================================
// MESSAGE 6571 PROCESSOR
// =============================================================================

func process6571Message(data []byte) {
	if len(data) < 298 { // 40-byte header + 258-byte message
		return
	}

	atomic.AddInt64(&message6571Count, 1)
...
func exportTo6571CSV(msg Message6571) {
	...
	csvWriter6571.Write(csvRecord)
	csvWriter6571.Flush()
	atomic.AddInt64(&message6571Saved, 1)
}
...
func printStats() {
	...
	msg6571 := atomic.LoadInt64(&message6571Count)
	saved := atomic.LoadInt64(&message6571Saved)
	...
	if duration > 0 {
		...
		fmt.Printf("⏱️  %.0fs | 📦 %d pkts (%.0f/s) | 🗜️  %d compressed | 🎯 6571: %s | %d msgs, %d saved\n", 
			duration, packets, float64(packets)/duration, compressed, status, msg6571, saved)
	}
}
...
func printFinalStats() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("FINAL STATISTICS - MESSAGE 6571 DECODER (BC_NORMAL_MKT_PREOPEN_ENDED)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n\n")
	...
	fmt.Printf("\n🎯 MESSAGE 6571 STATISTICS (BC_NORMAL_MKT_PREOPEN_ENDED)\n")
	fmt.Printf("  Total Messages:       %s\n", formatNumber(msg6571))
	...
	fmt.Printf("  Format: Preopen ended notifications\n")

	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	if msg6571 > 0 {
		fmt.Printf("✅ SUCCESS: Preopen Ended Messages (6571) processing completed\n")
		fmt.Printf("📊 Captured %d preopen transition notifications\n", saved)
	} else {
		fmt.Printf("⚠️  WARNING: No Preopen Ended Messages (6571) found during session\n")
		fmt.Printf("💡 Note: Preopen ended messages are broadcast at market transition (9:15 AM)\n")
	}
	fmt.Printf("✅ Check csv_output/ for message_6571_*.csv file\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
}
