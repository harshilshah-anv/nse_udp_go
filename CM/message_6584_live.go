...
// USAGE:
// ======
// Run: go run message_6584_live.go
// Output: csv_output/message_6584_TIMESTAMP.csv
//
...
// =============================================================================
// MESSAGE STRUCTURE FOR 6584
// =============================================================================

// Message6584 - BC_CLOSING_END (Closing Session End Messages)
// Per NSE CM Protocol - BCAST_VCT_MESSAGES structure
// Total packet: 298 bytes
type Message6584 struct {
	TransactionCode uint16   // Always 6584
	...
	Msg             [240]byte // 240 bytes - Closing session end message content
}
...
var (
	...
	message6584Count   int64
	message6584Saved   int64
	
	// CSV file for 6584
	csvFile6584        *os.File
	csvWriter6584      *csv.Writer
...
func main() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("NSE CM UDP Receiver - Message 6584 (BC_CLOSING_END)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
	fmt.Printf("Listening for message code 6584 (0x19C8 in hex)\n")
	fmt.Printf("Purpose: Closing session ended notification\n")
	fmt.Printf("Session: Post-Market end (4:00 PM completion)\n")
	fmt.Printf("Note: End of 20-minute closing price trading\n")
	...
	// Initialize CSV file for 6584
	if err := initialize6584CSV(); err != nil {
		log.Fatalf("Failed to initialize CSV: %v", err)
		return
	}
	defer func() {
		if csvWriter6584 != nil {
			csvWriter6584.Flush()
		}
		if csvFile6584 != nil {
			csvFile6584.Close()
...
func initialize6584CSV() error {
	timestamp := time.Now().Format("20060102_150405")
	filename := fmt.Sprintf("message_6584_%s.csv", timestamp)
	...
	csvFile6584 = file
	csvWriter6584 = csv.NewWriter(file)
...
func processUDPPacket(data []byte) {
	...
	// Offset 10-11: TransactionCode (SHORT 2 bytes) ← 6584 for BC_CLOSING_END
	...
	process6584Message(finalData)
}

// =============================================================================
// MESSAGE 6584 PROCESSOR
// =============================================================================

func process6584Message(data []byte) {
	if len(data) < 298 { // 40-byte header + 258-byte message
		return
	}

	atomic.AddInt64(&message6584Count, 1)
...
func exportTo6584CSV(msg Message6584) {
	...
	csvWriter6584.Write(csvRecord)
	csvWriter6584.Flush()
	atomic.AddInt64(&message6584Saved, 1)
}
...
func printStats() {
	...
	msg6584 := atomic.LoadInt64(&message6584Count)
	saved := atomic.LoadInt64(&message6584Saved)
	...
	if duration > 0 {
		...
		fmt.Printf("⏱️  %.0fs | 📦 %d pkts (%.0f/s) | 🗜️  %d compressed | 🎯 6584: %s | %d msgs, %d saved\n", 
			duration, packets, float64(packets)/duration, compressed, status, msg6584, saved)
	}
}
...
func printFinalStats() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("FINAL STATISTICS - MESSAGE 6584 DECODER (BC_CLOSING_END)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n\n")
	...
	fmt.Printf("\n🎯 MESSAGE 6584 STATISTICS (BC_CLOSING_END)\n")
	fmt.Printf("  Total Messages:       %s\n", formatNumber(msg6584))
	...
	fmt.Printf("  Format: Closing session end notifications\n")

	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	if msg6584 > 0 {
		fmt.Printf("✅ SUCCESS: Closing Session End Messages (6584) processing completed\n")
		fmt.Printf("📊 Captured %d closing session end notifications\n", saved)
	} else {
		fmt.Printf("⚠️  WARNING: No Closing Session End Messages (6584) found during session\n")
		fmt.Printf("💡 Note: Closing session end messages are broadcast at 4:00 PM\n")
	}
	fmt.Printf("✅ Check csv_output/ for message_6584_*.csv file\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
}
