// NSE Capital Market Multicast UDP Receiver - Message 6531 Only
// 
// FOCUS: Only process message code 6531 (BC_PREOPEN_SHUTDOWN_MSG - Preopen/Shutdown Messages)
// OUTPUT: csv_output/message_6531_TIMESTAMP.csv
//
// USAGE:
// ======
// Run: go run message_6531_live.go
// Output: csv_output/message_6531_TIMESTAMP.csv
//
// Protocol Reference: NSE CM NNF Protocol v6.3, Page 108-109 (Table 34)
// Structure: BCAST_VCT_MESSAGES (298 bytes)
// Contains: Market open/close/preopen status messages
...
func DecompressUltra(src []byte, dst []byte) (int, error) {
	// [Full LZO decompression code - same as 7207]
...
// =============================================================================
// MESSAGE STRUCTURE FOR 6531
// =============================================================================

// Message6531 - BC_PREOPEN_SHUTDOWN_MSG
// Per NSE CM Protocol Table 34 (Page 108-109)
// Total packet: 298 bytes
type Message6531 struct {
	TransactionCode uint16   // Always 6531
	Symbol          [6]byte  // 6 bytes
	Series          [2]byte  // 2 bytes  
	InstrumentType  int32    // 4 bytes
	MarketType      int16    // 2 bytes
	MsgLength       uint16   // 2 bytes
	Msg             [240]byte // 240 bytes
}
...
var (
	...
	message6531Count   int64
	message6531Saved   int64
	
	csvFile6531        *os.File
	csvWriter6531      *csv.Writer
...
func main() {
	...
	if err := initialize6531CSV(); err != nil {
		log.Fatalf("Failed to initialize 6531 CSV: %v", err)
		return
	}
...
func initialize6531CSV() error {
	timestamp := time.Now().Format("20060102_150405")
	filename := filepath.Join("csv_output", fmt.Sprintf("message_6531_%s.csv", timestamp))
	...
	csvFile6531 = file
	csvWriter6531 = csv.NewWriter(file)
	...
	csvWriter6531.Write(headers)
	csvWriter6531.Flush()

	fmt.Printf("📁 Created CSV file for Message 6531: %s\n", filename)
	return nil
}
...
func startUDPListener() {
	...
	fmt.Printf("\n╔════════════════════════════════════════════════════════════╗\n")
	fmt.Printf("║ NSE CM Message 6531 Receiver - Live Market Data          ║\n")
	fmt.Printf("╚════════════════════════════════════════════════════════════╝\n")
	fmt.Printf("📡 Multicast: %s:%d\n", multicastIP, port)
	fmt.Printf("🎯 Target: Message 6531 (BC_PREOPEN_SHUTDOWN_MSG)\n")
...
func processUDPPacket(data []byte) {
	...
	if transactionCode != 6531 {
		return
	}

	process6531Message(finalData)
}

// =============================================================================
// MESSAGE 6531 PROCESSOR
// =============================================================================

func process6531Message(data []byte) {
	if len(data) < 298 {
		return
	}

	atomic.AddInt64(&message6531Count, 1)
	currentCount := atomic.LoadInt64(&message6531Count)
	
	transactionCode := binary.BigEndian.Uint16(data[10:12])
	
	// Parse SEC_INFO (offset 40)
	var symbol [6]byte
	copy(symbol[:], data[40:46])
	
	var series [2]byte
	copy(series[:], data[46:48])
	
	instrumentType := int32(binary.BigEndian.Uint32(data[48:52]))
	marketType := int16(binary.BigEndian.Uint16(data[52:54]))
	...
	if currentCount == 1 {
		fmt.Printf("\n✅ First Message 6531 received\n\n")
	}
	
	exportToCSV(transactionCode, symbol, series, instrumentType, marketType, msgLength, msg)
	atomic.AddInt64(&message6531Saved, 1)
}

func exportToCSV(transactionCode uint16, symbol [6]byte, series [2]byte,
	instrumentType int32, marketType int16, msgLength uint16, msg [240]byte) {
	
	if csvWriter6531 == nil {
		return
	}

	sym := strings.TrimRight(string(symbol[:]), "\x00 ")
	ser := strings.TrimRight(string(series[:]), "\x00 ")
	...
	record := []string{
		...
		sym,
		ser,
		fmt.Sprintf("%d", instrumentType),
		fmt.Sprintf("%d", marketType),
		fmt.Sprintf("%d", msgLength),
		message,
	}

	csvWriter6531.Write(record)
	csvWriter6531.Flush()
}
...
func printStats() {
	...
	msg6531 := atomic.LoadInt64(&message6531Count)
	...
	if duration > 0 {
		...
		fmt.Printf("⏱️  %.0fs | 📦 %d pkts (%.0f/s) | 🗜️  %d compressed | 🎯 6531: %s | %d msgs\n", 
			duration, packets, float64(packets)/duration, compressed, status, msg6531)
	}
}
...
func printFinalStats() {
	...
	fmt.Println("\n" + strings.Repeat("=", 80))
	fmt.Println("📊 FINAL STATISTICS - MESSAGE 6531 DECODER")
	...
	fmt.Println(strings.Repeat("=", 80))
	fmt.Printf("Message 6531 Found     : %d messages\n", msg6531)
	fmt.Println(strings.Repeat("=", 80))
