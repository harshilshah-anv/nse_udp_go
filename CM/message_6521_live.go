...
// USAGE:
// ======
// Run: go run message_6521_live.go
// Output: csv_output/message_6521_TIMESTAMP.csv
//
...
// Message6521 - BC_CLOSE_MESSAGE (Market Close Messages)
// Per NSE CM Protocol - BCAST_VCT_MESSAGES structure
// Total packet: 298 bytes
type Message6521 struct {
	TransactionCode uint16   // Always 6521
	...
	Msg             [240]byte // 240 bytes - Market close message content
}
...
var (
	...
	message6521Count   int64
	message6521Saved   int64
	
	// CSV file for 6521
	csvFile6521        *os.File
	csvWriter6521      *csv.Writer
...
func main() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("NSE CM UDP Receiver - Message 6521 (BC_CLOSE_MESSAGE)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
	fmt.Printf("Listening for message code 6521 (0x1979 in hex)\n")
	fmt.Printf("Purpose: Market close notification\n")
	fmt.Printf("Session: Post-Market (3:30 PM close)\n")
	...
	// Initialize CSV file for 6521
	if err := initialize6521CSV(); err != nil {
		log.Fatalf("Failed to initialize CSV: %v", err)
		return
	}
	defer func() {
		if csvWriter6521 != nil {
			csvWriter6521.Flush()
		}
		if csvFile6521 != nil {
			csvFile6521.Close()
...
func initialize6521CSV() error {
	timestamp := time.Now().Format("20060102_150405")
	filename := fmt.Sprintf("message_6521_%s.csv", timestamp)
	...
	csvFile6521 = file
	csvWriter6521 = csv.NewWriter(file)
...
func processUDPPacket(data []byte) {
	...
	// Offset 10-11: TransactionCode (SHORT 2 bytes) ← 6521 for BC_CLOSE_MESSAGE
	...
	process6521Message(finalData)
}

// =============================================================================
// MESSAGE 6521 PROCESSOR
// =============================================================================

func process6521Message(data []byte) {
	if len(data) < 298 { // 40-byte header + 258-byte message
		return
	}

	atomic.AddInt64(&message6521Count, 1)
...
func exportTo6521CSV(msg Message6521) {
	...
	csvWriter6521.Write(csvRecord)
	csvWriter6521.Flush()
	atomic.AddInt64(&message6521Saved, 1)
}
...
func printStats() {
	...
	msg6521 := atomic.LoadInt64(&message6521Count)
	saved := atomic.LoadInt64(&message6521Saved)
	...
	if duration > 0 {
		...
		fmt.Printf("⏱️  %.0fs | 📦 %d pkts (%.0f/s) | 🗜️  %d compressed | 🎯 6521: %s | %d msgs, %d saved\n", 
			duration, packets, float64(packets)/duration, compressed, status, msg6521, saved)
	}
}
...
func printFinalStats() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("FINAL STATISTICS - MESSAGE 6521 DECODER (BC_CLOSE_MESSAGE)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n\n")
	...
	fmt.Printf("\n🎯 MESSAGE 6521 STATISTICS (BC_CLOSE_MESSAGE)\n")
	fmt.Printf("  Total Messages:       %s\n", formatNumber(msg6521))
	...
	fmt.Printf("  Format: Market close notifications\n")

	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	if msg6521 > 0 {
		fmt.Printf("✅ SUCCESS: Market Close Messages (6521) processing completed\n")
		fmt.Printf("📊 Captured %d market close notifications\n", saved)
	} else {
		fmt.Printf("⚠️  WARNING: No Market Close Messages (6521) found during session\n")
		fmt.Printf("💡 Note: Market close messages are broadcast at market close (3:30 PM)\n")
	}
	fmt.Printf("✅ Check csv_output/ for message_6521_*.csv file\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
}
