...
// USAGE:
// ======
// Run: go run message_6581_live.go
// Output: csv_output/message_6581_TIMESTAMP.csv
//
// Protocol Reference: NSE CM NNF Protocol v6.3
// Structure: BCAST_VCT_MESSAGES (298 bytes)
// Session: Auction Session Status Change Notifications
...
// =============================================================================
// MESSAGE STRUCTURE FOR 6581
// =============================================================================

// Message6581 - BC_AUCTION_STATUS_CHANGE (Auction Status Change Messages)
// Per NSE CM Protocol - BCAST_VCT_MESSAGES structure
// Total packet: 298 bytes
type Message6581 struct {
	TransactionCode uint16   // Always 6581
	...
	Msg             [240]byte // 240 bytes - Auction status change message content
}
...
var (
	...
	message6581Count   int64
	message6581Saved   int64
	
	// CSV file for 6581
	csvFile6581        *os.File
	csvWriter6581      *csv.Writer
...
func main() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("NSE CM UDP Receiver - Message 6581 (BC_AUCTION_STATUS_CHANGE)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
	fmt.Printf("Listening for message code 6581 (0x19C5 in hex)\n")
	fmt.Printf("Purpose: Auction session status change notification\n")
	fmt.Printf("Session: Auction Session Management\n")
	fmt.Printf("Note: Status transitions between auction phases\n")
	...
	// Initialize CSV file for 6581
	if err := initialize6581CSV(); err != nil {
...
func initialize6581CSV() error {
	timestamp := time.Now().Format("20060102_150405")
	filename := fmt.Sprintf("message_6581_%s.csv", timestamp)
	...
	csvFile6581 = file
	csvWriter6581 = csv.NewWriter(file)
	...
	for {
		...
		case <-shutdownChan:
			...
			if len(packetBuffer) > 0 {
				fmt.Printf("📦 Processing %d remaining packets before shutdown\n", len(packetBuffer))
		...
		case <-ticker.C:
			if len(packetBuffer) > 0 {
				...
				packetBuffer = packetBuffer[:0] // Reset buffer
			}
		}
	}
}
...
func processUDPPacket(data []byte) {
	...
	// Offset 10-11: TransactionCode (SHORT 2 bytes) ← 6581 for BC_AUCTION_STATUS_CHANGE
	...
	process6581Message(finalData)
}

// =============================================================================
// MESSAGE 6581 PROCESSOR
// =============================================================================

func process6581Message(data []byte) {
	if len(data) < 298 { // 40-byte header + 258-byte message
		return
	}

	atomic.AddInt64(&message6581Count, 1)
...
func exportTo6581CSV(msg Message6581) {
	...
	csvWriter6581.Write(csvRecord)
	csvWriter6581.Flush()
	atomic.AddInt64(&message6581Saved, 1)
}
...
func printStats() {
	...
	msg6581 := atomic.LoadInt64(&message6581Count)
	saved := atomic.LoadInt64(&message6581Saved)
	...
	if duration > 0 {
		...
		fmt.Printf("⏱️  %.0fs | 📦 %d pkts (%.0f/s) | 🗜️  %d compressed | 🎯 6581: %s | %d msgs, %d saved\n", 
			duration, packets, float64(packets)/duration, compressed, status, msg6581, saved)
	}
}
...
func printFinalStats() {
	...
	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	fmt.Printf("FINAL STATISTICS - MESSAGE 6581 DECODER (BC_AUCTION_STATUS_CHANGE)\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n\n")
	...
	fmt.Printf("\n🎯 MESSAGE 6581 STATISTICS (BC_AUCTION_STATUS_CHANGE)\n")
	fmt.Printf("  Total Messages:       %s\n", formatNumber(msg6581))
	...
	fmt.Printf("  Format: Auction status change notifications\n")

	fmt.Printf("\n" + strings.Repeat("=", 80) + "\n")
	if msg6581 > 0 {
		fmt.Printf("✅ SUCCESS: Auction Status Change Messages (6581) processing completed\n")
		fmt.Printf("📊 Captured %d auction status change notifications\n", saved)
	} else {
		fmt.Printf("⚠️  WARNING: No Auction Status Change Messages (6581) found during session\n")
		fmt.Printf("💡 Note: Auction status messages are broadcast during auction sessions\n")
	}
	fmt.Printf("✅ Check csv_output/ for message_6581_*.csv file\n")
	fmt.Printf(strings.Repeat("=", 80) + "\n")
}
